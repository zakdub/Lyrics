class DataFetcher {
    ...
    async getSongInfos(name, limit = 1) {
        ...
        await Promise.all(validSongs.map((song) => song.resolveAlbumCover(this)));
    ...
    async getTrackById(mbid) {
        ...
        if (result.track) {
            const song = new Song(result.track);
            await song.resolveAlbumCover(this);
            return song;
        }
    ...
    /**
     * Fetches the front cover art for an album release from the Cover Art
     * Archive, using the release's MusicBrainz ID. Last.fm's own album
     * images are frequently blank, so this is the primary source of covers.
     *
     * @private
     * @param {string} mbid
     * @returns {string|null} the URL of the front cover image, or null if unavailable
     */
    async getAlbumCoverArt(mbid) {
        if (!mbid) return null;
        ...
        try {
            ...
            return frontImage?.thumbnails?.large || frontImage?.image || null;
        } catch (err) {
            console.error('Failed to fetch cover art for release', mbid, err);
            return null;
        }
    }
